...
+   [JUC工具类: Exchanger详解](#juc%e5%b7%a5%e5%85%b7%e7%b1%bb-exchanger%e8%af%a6%e8%a7%a3)
    +   [带着BAT大厂的面试问题去理解Exchanger](#%e5%b8%a6%e7%9d%80%e9%97%ae%e9%a2%98%e5%8e%bb%e7%90%86%e8%a7%a3exchanger)
    +   [Exchanger简介](#exchanger%e7%ae%80%e4%bb%8b)
    +   [Exchanger实现机制](#exchanger%e5%ae%9e%e7%8e%b0%e6%9c%ba%e5%88%b6)
    +   [Exchanger源码解析](#exchanger%e6%ba%90%e7%a0%81%e8%a7%a3%e6%9e%90)
        +   [内部类 - Participant](#%e5%86%85%e9%83%a8%e7%b1%bb---participant)
        +   [内部类 - Node](#%e5%86%85%e9%83%a8%e7%b1%bb---node)
        +   [核心属性](#%e6%a0%b8%e5%bf%83%e5%b1%9e%e6%80%a7)
        +   [构造函数](#%e6%9e%84%e9%80%a0%e5%87%bd%e6%95%b0)
        +   [核心方法 - exchange(V x)](#%e6%a0%b8%e5%bf%83%e6%96%b9%e6%b3%95---exchangev-x)
...
NULL\_ITEM 为一个空节点，其实就是一个Object对象而已，slotExchange()为单个slot交换。
...
在自旋+阻塞模式中，首先取得结束时间和自旋次数。如果match(做releasing操作的线程传递的项)为null，其首先尝试spins+随机次自旋(改自旋使用当前节点中的hash，并改变之)和退让。当自旋数为0后，假如slot发生了改变(slot != p)则重置自旋数并重试。否则假如：当前未中断&arena为null&(当前不是限时版本或者限时版本+当前时间未结束)：阻塞或者限时阻塞。假如：当前中断或者arena不为null或者当前为限时版本+时间已经结束：不限时版本：置v为null；限时版本：如果时间结束以及未中断则TIMED\_OUT；否则给出null(原因是探测到arena非空或者当前线程中断)。
...
+   引用 https://www.pdai.tech/md/java/thread/java-thread-x-juc-tool-exchanger.html